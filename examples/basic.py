...
from llm_toolchain import Toolchain , PromptAdapter , tools, SemanticToolSelector
...
def main():
    ...
    # 3. Create an instance of the Prompt adapter. This adapter uses prompting to communicate with almost any LLM.
    # In case you need to add specific paths, you can use the manual run path or parse path.
    # For example, if the LLM is run by calling llm.chat.invoke() just call it with manual_run_path = ("chat", "invoke")
    # If the response is in response.choices[0].message.content use manual_parse_path = ("choices", 0, "message", "content")
    # We are still working on making it work with different payloads. 
    geminiadapter = PromptAdapter()
